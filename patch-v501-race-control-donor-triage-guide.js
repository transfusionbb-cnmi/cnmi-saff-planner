...
(function(){
  'use strict';
  const VERSION='V501_RACE_CONTROL_DONOR_TRIAGE_GUIDE';
  if(window.__CNMI_V501_RACE_CONTROL_DONOR_TRIAGE_GUIDE__) return;
  window.__CNMI_V501_RACE_CONTROL_DONOR_TRIAGE_GUIDE__=true;
  ...
  const phaseStore=window.__CNMI_V501_RACE_PHASES__||(window.__CNMI_V501_RACE_PHASES__={});
  ...
  function physicianDaytimePeople(date,unavailable,warnings){
    ...
    slots.forEach(([key,label])=>{
      ...
      if(!sid)return;
      if(unavailable.full.has(sid)){warnings.push(`${label}: ${staffName(sid)} ไม่อยู่หน้างาน (${unavailable.reasons.get(sid)||'ลา/อบรม/กิจกรรม'})`);return;}
      const p=unavailable.partial.get(sid);
      if(p){warnings.push(`${label}: ${staffName(sid)} ลาครึ่ง${p==='morning'?'เช้า':'บ่าย'} — ตรวจผู้แทนตามช่วงเวลา`);return;}
      if(seen.has(sid)){warnings.push(`${label}: ใช้แพทย์คนเดียวกับอีกจุด — RACE นับบุคคลเพียง 1 ครั้ง`);return;}
      seen.add(sid);
      out.push({staff_id:sid,label,slot:key});
    });
    return out;
  }
  ...
  function daytimePlan(date){
    ...
    allRows.forEach(row=>{
      ...
      const p=unavailable.partial.get(sid);
      const item={row,staff_id:sid,label:codeLabel(row),partial:p||'',zone:positionZone(positionCode(row))};
      availableRows.push(item);
      if(seenStaff.has(sid))warnings.push(`${staffName(sid)} ถูกจัดมากกว่า 1 ตำแหน่ง — RACE จะนับบุคคลเพียง 1 ครั้ง`);
      seenStaff.add(sid);
    ...
    if(parichat&&availableRows.some(x=>String(x.staff_id)===String(parichat.id))){leaderId=String(parichat.id);leaderNote='หัวหน้าหน่วย · อยู่หน้างาน';}
    if(!leaderId){
    ...
    if(!leaderId&&availableRows[0]){leaderId=String(availableRows[0].staff_id);leaderNote='ผู้แทนหัวหน้าทีม';}

    const pool=availableRows.filter(x=>String(x.staff_id)!==String(leaderId));
    const used=new Set();
    const rowBy=(codes)=>{
    ...
    const take=(role,item,secondary='')=>{
      ...
      const p={staff_id:String(item.staff_id),label:item.label,secondary:secondary||(item.partial?`${item.label} · ลาครึ่ง${item.partial==='morning'?'เช้า':'บ่าย'}`:item.label)};
      addRole(roles,role,p);used.add(String(item.staff_id));return true;
    };

    // R — Rescue: จุดเจาะเก็บ + แพทย์ Consult ที่อยู่หน้างาน
    take('R',rowBy(['drmain1']),'DR-Main 1 · จุดเจาะเก็บ');
    take('R',rowBy(['drmain2']),'DR-Main 2 · จุดเจาะเก็บ');

    // A — Alarm: แยก coverage 2 โซน แต่ไม่ให้คนเดียวทำซ้ำ
    const alarmFront=rowBy(['drregister','drregistration','drfinger','drsupport']);
    take('A',alarmFront,alarmFront?`${alarmFront.label} · ด้านหน้า`:'' );
    const alarmBack=rowBy(['bbsupport','bbreport','bbapprove','bbstockissue']);
    take('A',alarmBack,alarmBack?`${alarmBack.label} · ด้านหลัง`:'' );

    // C — Control: เป้าหมาย 4 คน แบ่ง 2 จุด Blood Bank + 2 จุด Component Prep/Manual
    // คนละคน ไม่ซ้ำ R/A/E และเลือก fallback ภายในโซนก่อน
    let controlBlood1=rowBy(['bbstockissue']);
    if(!controlBlood1)controlBlood1=rowBy(['bbreport','bbapprove']);
    take('C',controlBlood1,controlBlood1?`${controlBlood1.label} · Blood Bank 1`:'' );
    let controlBlood2=rowBy(['bbreport']);
    if(!controlBlood2)controlBlood2=rowBy(['bbstockissue','bbapprove']);
    take('C',controlBlood2,controlBlood2?`${controlBlood2.label} · Blood Bank 2`:'' );

    let controlManual1=rowBy(['bbmanual3']);
    if(!controlManual1)controlManual1=rowBy(['bbmanual2','bbmanual1','bbmanual4']);
    take('C',controlManual1,controlManual1?`${controlManual1.label} · Component Prep 1`:'' );
    let controlManual2=rowBy(['bbmanual4']);
    if(!controlManual2)controlManual2=rowBy(['bbmanual2','bbmanual1','bbmanual3']);
    take('C',controlManual2,controlManual2?`${controlManual2.label} · Component Prep 2`:'' );

    // แพทย์ทั้ง 2 จุดช่วย Rescue ผู้บริจาค/ผู้ที่ต้องการความช่วยเหลือได้
    const doctors=physicianDaytimePeople(date,unavailable,warnings);
    doctors.forEach(p=>{
    ...
    // E — Evacuate: บุคลากรที่เหลือทั้งหมด รับหน้าที่อพยพ/ตรวจผู้ตกค้าง
    pool.forEach(item=>{
      if(used.has(String(item.staff_id)))return;
      take('E',item,`${item.label} · อพยพ/ตรวจผู้ตกค้าง`);
    });

    // ถ้าจุดหลักขาด ให้แจ้งชัด ไม่ดึงคนจากอีกบทบาทมาซ้ำโดยอัตโนมัติ
    if(!roles.A.some(p=>String(p.secondary||'').includes('ด้านหน้า')))warnings.push('Alarm ด้านหน้าไม่มีผู้รับผิดชอบ — ตรวจ DR-Register/ผู้แทน');
    if(!roles.A.some(p=>String(p.secondary||'').includes('ด้านหลัง')))warnings.push('Alarm ด้านหลังไม่มีผู้รับผิดชอบ — ตรวจ BB-Support/ผู้แทน');
    if(roles.C.length<4)warnings.push(`Control ได้ ${roles.C.length}/4 คน — เป้าหมาย Blood Bank 2 + Component Prep 2; ตรวจผู้แทนตามพื้นที่`);
    if(roles.R.length<2)warnings.push('Rescue จุดผู้บริจาคมีผู้รับผิดชอบน้อย — ตรวจ DR-Main/แพทย์ผู้แทน');
    ROLE_ORDER.forEach(r=>{if(!roles[r].length)warnings.push(`${ROLE_META[r].title} ยังไม่มีผู้รับผิดชอบในแผนกลางวัน`);});

    const teamIds=uniquePeople([...availableRows.map(x=>({staff_id:x.staff_id})),...doctors]);
    return {id:'day',label:'กลางวัน',sub:'อิงตำแหน่งกลางวันจริง',leaderId,leaderNote,roles,roleNotes:{},roleTips:{R:'กลุ่มอพยพผู้บริจาค: กำลังเจาะ/ยังต้องมีคนช่วยก่อนเคลื่อนย้าย = เหลือง · หลังถอดเข็มและเดินได้เอง = เขียว'},warnings,teamCount:teamIds.length,note:'หัวหน้าทีมแยกจาก R/A/C/E · Alarm แยกด้านหน้า/ด้านหลัง · Control เป้าหมาย 4 คน (Blood Bank 2 + Component Prep 2) · แพทย์ Consult ที่อยู่หน้างานช่วย Rescue · บุคลากรที่เหลือเป็น Evacuate'};
  }
  ...
  if(typeof previous==='function'){
    const wrapped=function renderDashboardV500(){return decorateHtml(previous.apply(this,arguments));};
  ...
  window.cnmiV501Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate};
