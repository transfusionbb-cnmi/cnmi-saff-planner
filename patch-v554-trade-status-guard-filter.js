...
(function(){
  ...
  function setVersion(root=document){
    root.querySelectorAll?.('.v520-version-chip').forEach(chip=>{chip.textContent='v554';});
  }
  ...
  setVersion(document);
  const observer=new MutationObserver(()=>setVersion(document));
  observer.observe(document.documentElement,{childList:true,subtree:true});
  console.info(`${VERSION} trade status guard/filter loaded`);
})();
