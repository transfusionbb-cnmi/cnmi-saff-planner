/* CNMI Staff Planner V530 — Fractional HR Unit + Strict Sidebar Accordion
...
(function(){
  'use strict';
  if(window.__CNMI_V530_FRACTIONAL_HR_UNIT_ACCORDION__) return;
  window.__CNMI_V530_FRACTIONAL_HR_UNIT_ACCORDION__=true;
  const VERSION='V530_FRACTIONAL_HR_UNIT_ACCORDION';
  ...
  function decorateVersion(){
    ...
    if(chip){chip.textContent='v530';chip.title='Fractional HR unit + strict sidebar accordion';chip.classList.add('v530-version-chip');}
  }
  function start(){decorateVersion();const foot=document.querySelector('.sidebar-foot');if(foot)new MutationObserver(decorateVersion).observe(foot,{childList:true,subtree:true,characterData:true});}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
  window.cnmiV530={version:VERSION,collapseAll};
