 ...
 * - V547: RACE now consumes the SAME effectiveEntriesForStaffDate source used by the duty table, instead of independently rebuilding trade ownership.
 * - V547: if the duty table shows a seller remainder / receiver segment, RACE uses that exact effective segment as its source of truth.
 * - V547: startup/PWA/mobile recovery files are intentionally untouched.
 * Display-only; no Supabase schema/write changes.
 */
(function(){
  'use strict';
  const VERSION='V547_RACE_EFFECTIVE_ROSTER_SOURCE';
  if(window.__CNMI_V547_RACE_EFFECTIVE_ROSTER_SOURCE__) return;
  window.__CNMI_V547_RACE_EFFECTIVE_ROSTER_SOURCE__=true;
  ...
  const phaseStore=window.__CNMI_V547_RACE_PHASES__||(window.__CNMI_V547_RACE_PHASES__={});
  ...
  function completedTradeRowsForDate(date){
    return (S().tradeRequests||[]).filter(r=>String(r?.status||'').toLowerCase()==='completed').filter(r=>{
      const aid=String(r?.from_assignment_id||'');
      return rawDutyRows(date).some(a=>String(a?.id||'')===aid);
    });
  }
  function allEffectiveStaffIds(date){
    const ids=new Set();
    rawDutyRows(date).forEach(a=>{if(a?.staff_id)ids.add(String(a.staff_id));});
    completedTradeRowsForDate(date).forEach(r=>{if(r?.requester_id)ids.add(String(r.requester_id));if(r?.receiver_id)ids.add(String(r.receiver_id));});
    return [...ids];
  }
  ...
  window.cnmiV547Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate,reportRosterHtml,effectiveDutyRows,tradeBoundaries,rawDutyRows,effectiveDutySegments};
  window.cnmiV505Race=window.cnmiV547Race;
