/* CNMI Staff Planner V367 — continuous OT carry with source data ready for summary and HR export
...
(function(){
  'use strict';
  if(window.__CNMI_V367_CONTINUOUS_OT_CARRY_SOURCE_READY__)return;
  window.__CNMI_V367_CONTINUOUS_OT_CARRY_SOURCE_READY__=true;

  const VERSION='V367_CONTINUOUS_OT_CARRY_SOURCE_READY';
  ...
  async function preloadRateSources(lastMonth,force=false){
    const api=window.cnmiV348;
    if(!api)return;
    let month=ANCHOR_MONTH;
    while(month<=lastMonth){
      try{if(typeof api.ensureHelpers==='function')await api.ensureHelpers(month,{force:!!force});}catch(err){console.warn('[V367] helper source preload failed',month,err);}
      try{if(typeof api.ensureTrades==='function')await api.ensureTrades(month);}catch(err){console.warn('[V367] trade source preload failed',month,err);}
  ...
  async function continuousCarry(targetMonth,force=false){
    ...
    // The monthly table uses donor-helper slot and completed-trade data when
    // converting each OT row to HR-equivalent hours. Load those same sources
    // before recalculating prior months; otherwise carry-in can differ from the
    // previous month's visible carry-out (for example 7.60 instead of 5.14).
    await preloadRateSources(last,force);
  ...
  function install(){
    ...
    if(api.queryCarryInSummary.__v367&&api.queryCarryIn.__v367)return true;
    ...
    const wrap=fallback=>{
      const fn=async function(month,force){
        ...
        catch(err){console.warn('[V367] continuous carry failed; using saved fallback',err);return fallback(month,force);}
      };
      fn.__v367=true;return fn;
    };
    ...
    api.version=VERSION;
    window.dispatchEvent(new CustomEvent('cnmi:v367-ready'));
    return true;
  }
  if(!install()){
