...
(function(){
  'use strict';
  const VERSION='V505_RACE_ACCOUNTABILITY_FULLNAME_HELPER_ROSTER';
  if(window.__CNMI_V505_RACE_ACCOUNTABILITY_FULLNAME_HELPER_ROSTER__) return;
  window.__CNMI_V505_RACE_ACCOUNTABILITY_FULLNAME_HELPER_ROSTER__=true;
  ...
  const phaseStore=window.__CNMI_V505_RACE_PHASES__||(window.__CNMI_V505_RACE_PHASES__={});
  ...
  function staffHtml(id,extra=''){
    ...
    return `<span class="v497-person"><button type="button" class="v505-name-tap" data-v505-person-id="${esc(id)}" aria-label="ดูชื่อจริง ${esc(staffName(id))}">${html}</button>${extra?`<small>${esc(extra)}</small>`:''}</span>`;
  }
  ...
  function dutyPlan(date,kind){
    ...
    const reportPeople=uniqueReportPeople([
      ...included.map(a=>reportStaffPerson(a.staff_id,window.DUTY_LABEL?.[dutyCode(a)]||dutyCode(a))),
    ...
    return {id:kind,label,sub,leaderId:leader.id,leaderNote:leader.note,roles,roleNotes,roleTips,warnings,teamCount:included.length,reportPeople,reportCountText,reportRosterNote:kind==='weekend-donor'?(helperInfo.loaded?'รวมคนมาช่วยในยอดตรวจนับ/รายงานศูนย์อำนวยการ แม้ไม่ถูกจัด RACE':(helperInfo.error?'ต้องตรวจรายชื่อคนมาช่วยหน้างานเพิ่ม':'กำลังตรวจรายชื่อคนมาช่วย…')):'หัวหน้าทีมตรวจนับคนจริง ณ เวลาที่เกิดเหตุ และรายงานศูนย์อำนวยการ',note:[note,leaderRule].filter(Boolean).join(' · ')};
  }
  ...
  function reportRosterHtml(plan){
    ...
    const rows=sorted.map(p=>{
      if(p.kind==='staff'||p.staff_id){
        ...
        return `<div class="v505-report-person"><button type="button" data-v505-person-id="${esc(id)}"><b>${esc(full)}</b><span>${esc(nick&&nick!==full?`(${nick}) · ${role}`:role)}</span></button>${p.note?`<small>${esc(p.note)}</small>`:''}</div>`;
      }
  ...
  function planHtml(plan,date){
    return `<div class="v497-plan" data-v497-plan="${esc(plan.id)}">
      ...
      <div class="v505-tap-hint">แตะชื่อเล่นเพื่อดูชื่อจริง • หัวหน้าทีมใช้รายชื่อด้านล่างตรวจนับและรายงานศูนย์อำนวยการ</div>
  ...
  function showPersonFullName(id){
    const p=staffById(id)||{};
    const nick=staffName(id),full=staffFullName(id);
    const type=txt(p?.staff_type||p?.position||p?.job_title||'');
    const body=`<div class="v505-name-modal"><span>ชื่อจริง</span><strong>${esc(full)}</strong>${nick&&nick!==full?`<small>ชื่อเล่น: ${esc(nick)}</small>`:''}${type?`<small>${esc(type)}</small>`:''}</div>`;
    try{showModal(`<h2>ข้อมูลบุคลากร</h2>${body}`);}catch(_){alert(full);}
  }

  document.addEventListener('click',e=>{
    const person=e.target.closest?.('[data-v505-person-id]');
    if(person){e.preventDefault();e.stopPropagation();showPersonFullName(person.getAttribute('data-v505-person-id'));return;}
  ...
  window.cnmiV505Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate,reportRosterHtml};
