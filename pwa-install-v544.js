/* CNMI Staff Planner PWA registration + install prompt — V544 */
(() => {
  'use strict';
  ...
  function ensureAuthButton() {
    ...
    if (!authCard || authCard.querySelector('[data-cnmi-pwa-auth-install="v544"]')) return;
    const wrapper = document.createElement('div');
    wrapper.dataset.cnmiPwaAuthInstall = 'v544';
  ...
  function ensureMenuButton() {
    ...
    if (!sidebarFoot || sidebarFoot.querySelector('[data-cnmi-pwa-menu-install="v544"]')) return;
    const button = createButton('pwa-install-btn--menu');
    button.dataset.cnmiPwaMenuInstall = 'v544';
  ...
  function createModal() {
    if (document.getElementById('pwaInstallModalV544')) return;
    const modal = document.createElement('div');
    modal.id = 'pwaInstallModalV544';
    modal.className = 'pwa-install-modal hidden';
    modal.setAttribute('role', 'dialog');
    modal.setAttribute('aria-modal', 'true');
    modal.setAttribute('aria-labelledby', 'pwaInstallTitleV544');
    modal.innerHTML = `
      <div class="pwa-install-dialog">
        <button type="button" class="pwa-install-close" aria-label="ปิด">×</button>
        <h3 id="pwaInstallTitleV544">ติดตั้ง CNMI Staff Planner</h3>
  ...
  function showModal(html) {
    createModal();
    const modal = document.getElementById('pwaInstallModalV544');
    if (!modal) return;
  ...
  async function handleInstallClick() {
    ...
    if (deferredInstallPrompt) {
      ...
      try {
        ...
        console.warn('[PWA V544] Install prompt failed:', error);
  ...
  async function registerServiceWorker() {
    ...
    try {
      const workerUrl = new URL('sw.js?v=544', scriptBaseUrl);
      ...
      console.info('[PWA V544] Service Worker registered:', registration.scope);
    } catch (error) {
      console.warn('[PWA V544] Service Worker registration failed:', error);
    }
  }
