/* CNMI Staff Planner V545 — Startup dependency failover
...
(function(){
  'use strict';
  if(window.__CNMI_V545_DEPENDENCY_BOOTSTRAP__) return;
  window.__CNMI_V545_DEPENDENCY_BOOTSTRAP__=true;

  const LOAD_TIMEOUT=6500;
  const CACHE_NAME='cnmi-external-deps-v545';
  ...
  function connectionNote(show){
    let note=document.getElementById('v545ConnectionNote');
    if(!show){note?.remove();return;}
    if(note) return;
    const form=document.getElementById('loginForm');
    if(!form) return;
    note=document.createElement('div');
    note.id='v545ConnectionNote';
  ...
  window.CNMI_DEPENDENCY_READY={version:'v545',supabase:supabasePromise,ensure,state};
  window.cnmiEnsureDependency=ensure;

  function warmOptional(){
    ['xlsx','html2canvas','pdf-lib','jszip'].forEach(name=>ensure(name).catch(err=>{
      console.warn(`[V545] optional dependency ${name} unavailable; it can retry later`,err?.message||err);
    }));
    ...
    try{
      if(!document.querySelector('link[data-v545-google-fonts]')){
        const link=document.createElement('link');
        link.rel='stylesheet';
        link.dataset.v545GoogleFonts='1';
