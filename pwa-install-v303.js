...
(() => {
  ...
  async function registerServiceWorker() {
    ...
    try {
      const workerUrl = new URL('sw.js?v=319', scriptBaseUrl);
      const registration = await navigator.serviceWorker.register(workerUrl.href, { scope: scriptBaseUrl.pathname });
      registrationReady = true;
      registration.update().catch(() => {});
      console.info('[PWA V319] Service Worker registered:', registration.scope);
    } catch (error) {
      console.warn('[PWA V319] Service Worker registration failed:', error);
    }
  }
