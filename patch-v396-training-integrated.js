...
(function () {
  ...
  function myTrainingStatus(){return Object.prototype.hasOwnProperty.call(S(),'v402MyStatus')?String(S().v402MyStatus||''):'';}
  ...
  function myTrainingFilters(){
    const selected=myTrainingStatus(),year=selectedTrainingYear(),ready=isMyTrainingFilterReady(),annualRows=ready?myTrainingAnnualRows():[],certificateCount=annualRows.filter(x=>x.record.certificate_path).length,canExport=ready&&annualRows.length>0;
    return `<div class="toolbar compact-filter v396-filters v402-my-training-filters v407-year-filter v416-filter-gate">
      <label>สถานะข้อมูล <select id="v402MyStatus"><option value="" ${!selected?'selected':''} disabled>เลือกสถานะข้อมูล</option><option value="รอกรอกข้อมูล" ${selected==='รอกรอกข้อมูล'?'selected':''}>รอกรอกข้อมูล</option><option value="กรอกข้อมูลแล้ว" ${selected==='กรอกข้อมูลแล้ว'?'selected':''}>กรอกข้อมูลแล้ว</option><option value="all" ${selected==='all'?'selected':''}>ทั้งหมด</option></select></label>
      <label>ปีแบบฟอร์ม <select id="v407TrainingYear"><option value="" ${!year?'selected':''} disabled>เลือกปีแบบฟอร์ม</option>${trainingYearOptions().map(y=>`<option value="${y}" ${y===year?'selected':''}>พ.ศ. ${y+543}</option>`).join('')}</select><small class="hint">${year?`ครอบคลุม 1 ม.ค. – 31 ธ.ค. ${year+543}`:'เลือกปีที่ต้องการดูก่อน'}</small></label>
      <div class="v402-export-wrap"><button class="primary-btn" type="button" data-v402-my-export ${canExport?'':'disabled'}>Export PDF ชุดอบรมประจำปี</button><small class="hint">${!ready?'เลือกตัวกรองให้ครบก่อน':annualRows.length?`รวมตาราง ${annualRows.length} รายการ และ Certificate ${certificateCount} ไฟล์`:'ไม่พบรายการสำหรับส่งออกในปีนี้'}</small></div>
    </div>`;
  }
  ...
  function renderMyTraining(){
    ...
    const ready=isMyTrainingFilterReady(),rows=ready?myTrainingRows():[],year=selectedTrainingYear();
    return `<div class="card v416-training-page"><div class="section-title"><div><h3>รายการอบรมของฉัน</h3><p class="hint">เลือกสถานะและปีแบบฟอร์มก่อน แล้วระบบจะแสดงเฉพาะรายการที่ตรงกับตัวกรอง</p></div></div>${trainingFormNotice()}${myTrainingFilters()}${!ready?trainingSelectionPrompt('เลือก “สถานะข้อมูล” และ “ปีแบบฟอร์ม” ก่อน'):rows.length?`<div class="v396-record-list">${rows.map(recordForm).join('')}</div>`:empty(`ไม่พบรายการอบรมในปี ${year+543} ตามตัวกรอง`)}</div>`;
  }
  ...
  async function exportMyAnnualTrainingPackage(items,button){
    ...
    try{
      ...
      if(built.errors.length){console.warn('V416 certificate merge skipped',built.errors);showToast(`สร้าง PDF แล้ว แต่แนบ Certificate ไม่สำเร็จ ${built.errors.length} ไฟล์`);}else showToast(`สร้าง PDF ชุดอบรมแล้ว • แนบ Certificate ${built.certificateCount} ไฟล์`);
    }catch(error){console.error('V416 annual training PDF',error);showToast(`สร้าง PDF ไม่สำเร็จ: ${error?.message||String(error)}`);}finally{if(button){button.disabled=false;button.textContent=originalText||'Export PDF ชุดอบรมประจำปี';}}
  }
  ...
  async function exportAdminAnnualTrainingPackage(button){
    ...
    try{
      ...
      if(errors.length){console.warn('V416 admin certificate merge skipped',errors);showToast(`Export สำเร็จ แต่แนบ Certificate ไม่สำเร็จ ${errors.length} ไฟล์`);}else showToast(`Export FM-CNHR-002 สำเร็จ ${groups.length} คน`);
    }catch(error){console.error('V416 admin annual training export',error);showToast(`Export ไม่สำเร็จ: ${error?.message||String(error)}`);}finally{if(button){button.disabled=false;button.textContent=originalText||(allPeople?'Export PDF ทุกคน':'Export PDF บุคลากรที่เลือก');}}
  }
  ...
  function resetTrainingFiltersOnEntry(page){
    const s=S(),year=currentGregorianYear();
    if(page==='myTraining'){s.v402MyStatus='';s.v407TrainingYear='';s.v396MyFrom='';s.v396MyTo='';}
    if(page==='trainingAdmin'){s.v396From=`${year}-01-01`;s.v396To=`${year}-12-31`;s.v396Staff='';s.v396Status='';}
  }
  function renderPageV396(){const p=S().page;if(p==='myTraining'||p==='trainingAdmin'){if(v416LastPage!==p)resetTrainingFiltersOnEntry(p);v416LastPage=p;const item=NAV_ITEMS.find(x=>x.id===p);$('pageTitle').textContent=item.title;$('pageSubtitle').textContent=item.subtitle;renderNav();$('pageContent').innerHTML=p==='myTraining'?renderMyTraining():renderTrainingAdmin();return;}v416LastPage=p;return oldRenderPage.apply(this,arguments);}
