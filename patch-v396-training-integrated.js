...
(function () {
  ...
  function myTrainingFilters(rows){const s=S(),selected=myTrainingStatus(),datesReady=Boolean(s.v396MyFrom&&s.v396MyTo),canExport=datesReady&&rows.length>0;return `<div class="toolbar compact-filter v396-filters v402-my-training-filters"><label>สถานะข้อมูล <select id="v402MyStatus"><option value="รอกรอกข้อมูล" ${selected==='รอกรอกข้อมูล'?'selected':''}>รอกรอกข้อมูล</option><option value="กรอกข้อมูลแล้ว" ${selected==='กรอกข้อมูลแล้ว'?'selected':''}>กรอกข้อมูลแล้ว</option><option value="" ${selected===''?'selected':''}>ทั้งหมด</option></select></label><label>ตั้งแต่ <input type="date" id="v396MyFrom" value="${esc(s.v396MyFrom||'')}"></label><label>ถึง <input type="date" id="v396MyTo" value="${esc(s.v396MyTo||'')}"></label><div class="v402-export-wrap"><button class="primary-btn" type="button" data-v402-my-export ${canExport?'':'disabled'}>Export PDF ตาราง FM-CNHR-002</button><small class="hint">${datesReady?(rows.length?'ส่งออกรายการตามตัวกรองเป็นตารางเดียว':'ไม่พบรายการสำหรับส่งออก'):'เลือกวันที่ตั้งแต่–ถึงก่อน Export PDF'}</small></div></div>`;}
  function myTrainingRows(){const s=S(),selected=myTrainingStatus();return rowItems().filter(x=>{if(!idEq(x.record.staff_id,actor()))return false;const start=dateKey(x.activity.start_date),end=dateKey(x.activity.end_date)||start;return(!selected||status(x.record)===selected)&&(!s.v396MyFrom||end>=s.v396MyFrom)&&(!s.v396MyTo||start<=s.v396MyTo);}).sort((a,b)=>dateKey(a.activity.start_date).localeCompare(dateKey(b.activity.start_date)));}
  function renderMyTraining(){if(!S().profile)return noPermission();const rows=myTrainingRows();return `<div class="card"><div class="section-title"><div><h3>รายการอบรมของฉัน</h3><p class="hint">กรองสถานะและช่วงวันที่ ก่อนส่งออกเป็นตารางแบบฟอร์ม FM-CNHR-002</p></div></div>${trainingFormNotice()}${myTrainingFilters(rows)}${rows.length?`<div class="v396-record-list">${rows.map(recordForm).join('')}</div>`:empty((S().v396MyFrom||S().v396MyTo||myTrainingStatus())?'ไม่พบรายการอบรมตามตัวกรอง':'ยังไม่มีรายการอบรมของคุณ')}</div>`;}
  ...
  function annualFormTitle(from,to){const fy=thaiYear(from),ty=thaiYear(to);return fy&&fy===ty?`แบบบันทึกการอบรมประจำปี ${fy}`:`แบบบันทึกการอบรม ${dateLabel(from)} – ${dateLabel(to)}`;}
  function organizerText(a){return a?.organizer||a?.organizer_name||a?.provider||a?.location||'-';}
  ...
  function printMyAnnualForm(items){const s=S(),from=s.v396MyFrom,to=s.v396MyTo;if(!from||!to)return showToast('กรุณาเลือกวันที่ตั้งแต่และถึงก่อน Export PDF');if(to<from)return showToast('วันที่สิ้นสุดต้องไม่ก่อนวันที่เริ่ม');if(!items.length)return showToast('ไม่พบรายการอบรมตามตัวกรอง');const profile=staffOf(actor()),personName=profile.full_name||profile.nickname||'-',employeeCode=profile.employee_code||profile.personnel_code||'-',position=profile.position||profile.staff_type||'-',startWork=employment(actor()),head=(S().staff||[]).find(x=>String(x.full_name||'').includes('ปาริฉัตร อินทร์เกลี้ยง'))||{},headName=head.full_name||'นางสาว ปาริฉัตร อินทร์เกลี้ยง',logoUrl=new URL('fm-cnhr-002-logo.png',window.location.href).href,pages=chunkRows(items,20),title=annualFormTitle(from,to);const sections=pages.map((page,pageIndex)=>{const offset=pageIndex*20,blank=Math.max(0,20-page.length),rows=page.map((x,i)=>`<tr><td class="center">${offset+i+1}</td><td>${esc(x.activity.title||'-')}</td><td></td><td>${esc(organizerText(x.activity))}</td><td class="center">${esc(dateLabel(x.activity.start_date))}</td><td class="center">${esc(dateLabel(x.activity.end_date||x.activity.start_date))}</td><td class="center">${esc(durationLabel(x.activity))}</td></tr>`).join('')+Array.from({length:blank},()=>'<tr class="blank"><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>').join('');return `<section class="fm-page"><table class="fm-header"><tr><td class="logo-cell" rowspan="3"><img class="fm-logo" src="${esc(logoUrl)}" alt="ตรามหาวิทยาลัยมหิดล"></td><td><b>ชื่อแบบฟอร์ม :</b> ${esc(title)}</td></tr><tr><td><b>ฝ่าย/งาน/หน่วย :</b> งานพยาธิ นิติเวช และบริการโลหิต</td></tr><tr><td>โรงพยาบาลรามาธิบดีจักรีนฤบดินทร์ คณะแพทยศาสตร์โรงพยาบาลรามาธิบดี มหาวิทยาลัยมหิดล</td></tr></table><table class="fm-person"><tr><td><b>ชื่อ สกุล :</b> ${esc(personName)}</td><td><b>รหัสบุคคล :</b> ${esc(employeeCode)}</td></tr><tr><td><b>ตำแหน่ง :</b> ${esc(position)}</td><td><b>วันเริ่มงาน :</b> ${esc(dateLabel(startWork))}</td></tr></table><table class="fm-training"><thead><tr><th>ที่</th><th>หลักสูตร/เรื่อง</th><th>รุ่น</th><th>หน่วยงานผู้จัด</th><th>วันที่เริ่มต้น</th><th>วันที่สิ้นสุด</th><th>ระยะเวลา</th></tr></thead><tbody>${rows}</tbody></table><div class="signatures"><div>ลงชื่อ ..........................................................<br><span>(${esc(personName)})</span><br>บุคลากรผู้รับการฝึกอบรม</div><div>ลงชื่อ ..........................................................<br><span>(${esc(headName)})</span><br>หัวหน้าหน่วย/หัวหน้างาน</div></div><div class="fm-footer"><span>หน้าที่ ${pageIndex+1} ของ ${pages.length}</span><span>FM-CNHR-002 Rev.00</span></div></section>`;}).join('');const w=window.open('','_blank');if(!w)return showToast('กรุณาอนุญาต Pop-up เพื่อ Export PDF');w.document.write(`<html><head><meta charset="utf-8"><title>FM-CNHR-002 ${esc(personName)}</title><style>@page{size:A4 portrait;margin:7mm}*{box-sizing:border-box}body{margin:0;font-family:"TH Sarabun New",Sarabun,Tahoma,sans-serif;color:#111;font-size:16pt;line-height:1.05}.fm-page{min-height:283mm;display:flex;flex-direction:column;page-break-after:always}.fm-page:last-child{page-break-after:auto}table{width:100%;border-collapse:collapse}.fm-header td,.fm-person td,.fm-training th,.fm-training td{border:1px solid #333}.fm-header td{padding:3px 6px;font-size:16pt;line-height:1.05}.logo-cell{width:92px;text-align:center}.fm-logo{display:block;width:76px;height:76px;object-fit:contain;margin:auto}.fm-person{margin-top:3px}.fm-person td{width:50%;padding:5px 8px;font-size:16pt;line-height:1.05}.fm-training{margin-top:6px;table-layout:fixed}.fm-training th{padding:4px 2px;font-size:16pt;line-height:1}.fm-training td{height:27px;padding:2px 4px;vertical-align:top;font-size:16pt;line-height:1}.fm-training th:nth-child(1){width:4%}.fm-training th:nth-child(2){width:36%}.fm-training th:nth-child(3){width:5%}.fm-training th:nth-child(4){width:23%}.fm-training th:nth-child(5){width:11%}.fm-training th:nth-child(6){width:11%}.fm-training th:nth-child(7){width:10%}.center{text-align:center}.blank td{height:24px}.signatures{margin-top:auto;border:1px solid #333;display:grid;grid-template-columns:1fr 1fr;gap:24px;padding:14px 20px 8px;text-align:center;font-size:16pt;line-height:1.2}.signatures span{display:inline-block;margin-top:3px}.fm-footer{display:flex;justify-content:space-between;padding:3px 2px 0;font-size:9pt;line-height:1}.fm-footer span:last-child{font-size:9pt}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>${sections}</body></html>`);w.document.close();setTimeout(()=>{w.focus();w.print();},700);}
  ...
  document.addEventListener('change',e=>{const m={v402MyStatus:'v402MyStatus',v396MyFrom:'v396MyFrom',v396MyTo:'v396MyTo',v396From:'v396From',v396To:'v396To',v396Staff:'v396Staff',v396Status:'v396Status'};if(m[e.target?.id]){S()[m[e.target.id]]=e.target.value||'';renderPage();}},true);
  ...
  const style=document.createElement('style');style.textContent='.v396-participants{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;max-height:240px;overflow:auto}.v396-participant{display:flex;gap:8px;padding:8px;border:1px solid #d9e4ed;border-radius:10px;background:#fff}.v396-participant input{width:auto}.v396-participant span{display:grid}.v396-participant small{color:#68798a}.v396-training-check{display:flex;gap:9px;align-items:flex-start;padding:12px;border:1px solid #9bc8e2;border-radius:12px;background:#f2faff;font-weight:700}.v396-training-check input{width:auto;margin-top:4px}.v396-training-check small{display:block;font-weight:400;color:#587087}.v396-record-list{display:grid;gap:14px;margin-top:14px}.v396-record textarea{width:100%;resize:vertical}.v396-filters{align-items:end}.v401-my-training-filters{grid-template-columns:repeat(2,minmax(180px,260px));justify-content:start}.v402-my-training-filters{display:grid;grid-template-columns:minmax(170px,220px) minmax(150px,210px) minmax(150px,210px) minmax(230px,1fr);align-items:end;gap:12px}.v402-export-wrap{display:grid;gap:5px;align-content:end}.v402-export-wrap button{width:100%}.v402-export-wrap button:disabled{opacity:.5;cursor:not-allowed}.v396-halfday{display:block;font-size:10px;font-weight:700}@media(max-width:900px){.v402-my-training-filters{grid-template-columns:1fr 1fr}.v402-export-wrap{grid-column:1/-1}}@media(max-width:700px){.v396-participants{grid-template-columns:1fr}.v396-filters{display:grid;grid-template-columns:1fr 1fr}.v396-filters label:last-child{grid-column:1/-1}.v401-my-training-filters label:last-child{grid-column:auto}.v402-my-training-filters label:last-of-type{grid-column:auto}}@media(max-width:460px){.v401-my-training-filters,.v402-my-training-filters{grid-template-columns:1fr}.v401-my-training-filters label:last-child,.v402-my-training-filters label:last-of-type,.v402-export-wrap{grid-column:1/-1}}';document.head.appendChild(style);
  loadTraining();
})();
