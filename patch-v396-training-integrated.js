...
(function () {
  ...
  async function exportMyAnnualTrainingPackage(items,button){
    ...
    try{
      ...
      if(built.errors.length){console.warn('V417 certificate merge skipped',built.errors);showToast(`สร้าง PDF แล้ว แต่แนบ Certificate ไม่สำเร็จ ${built.errors.length} ไฟล์`);}else showToast(`สร้าง PDF ชุดอบรมแล้ว • แนบ Certificate ${built.certificateCount} ไฟล์`);
    }catch(error){console.error('V417 annual training PDF',error);showToast(`สร้าง PDF ไม่สำเร็จ: ${error?.message||String(error)}`);}finally{if(button){button.disabled=false;button.textContent=originalText||'Export PDF ชุดอบรมประจำปี';}}
  }
  ...
  async function exportAdminAnnualTrainingPackage(button){
    ...
    try{
      ...
      if(errors.length){console.warn('V417 admin certificate merge skipped',errors);showToast(`Export สำเร็จ แต่แนบ Certificate ไม่สำเร็จ ${errors.length} ไฟล์`);}else showToast(`Export FM-CNHR-002 สำเร็จ ${groups.length} คน`);
    }catch(error){console.error('V417 admin annual training export',error);showToast(`Export ไม่สำเร็จ: ${error?.message||String(error)}`);}finally{if(button){button.disabled=false;button.textContent=originalText||(allPeople?'Export PDF ทุกคน':'Export PDF บุคลากรที่เลือก');}}
  }
