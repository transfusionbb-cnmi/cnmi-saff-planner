...
(function () {
  ...
  function myTrainingFilters(){const s=S();return `<div class="toolbar compact-filter v396-filters v401-my-training-filters"><label>ตั้งแต่ <input type="date" id="v396MyFrom" value="${esc(s.v396MyFrom||'')}"></label><label>ถึง <input type="date" id="v396MyTo" value="${esc(s.v396MyTo||'')}"></label></div>`;}
  function myTrainingRows(){const s=S();return rowItems().filter(x=>{if(!idEq(x.record.staff_id,actor()))return false;const start=dateKey(x.activity.start_date),end=dateKey(x.activity.end_date)||start;return(!s.v396MyFrom||end>=s.v396MyFrom)&&(!s.v396MyTo||start<=s.v396MyTo);}).sort((a,b)=>dateKey(b.activity.start_date).localeCompare(dateKey(a.activity.start_date)));}
  function renderMyTraining(){if(!S().profile)return noPermission();const rows=myTrainingRows();return `<div class="card"><div class="section-title"><div><h3>รายการอบรมของฉัน</h3><p class="hint">แสดงเฉพาะกิจกรรมที่ Admin ติ๊กนำเข้า FM-CNHR-002</p></div></div>${trainingFormNotice()}${myTrainingFilters()}${rows.length?`<div class="v396-record-list">${rows.map(recordForm).join('')}</div>`:empty((S().v396MyFrom||S().v396MyTo)?'ไม่พบรายการอบรมในช่วงวันที่ที่เลือก':'ยังไม่มีรายการอบรมของคุณ')}</div>`;}
  ...
  document.addEventListener('change',e=>{const m={v396MyFrom:'v396MyFrom',v396MyTo:'v396MyTo',v396From:'v396From',v396To:'v396To',v396Staff:'v396Staff',v396Status:'v396Status'};if(m[e.target?.id]){S()[m[e.target.id]]=e.target.value||'';renderPage();}},true);
