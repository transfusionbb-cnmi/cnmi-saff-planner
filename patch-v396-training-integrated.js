...
(function () {
  ...
  async function exportMyAnnualTrainingPackage(items,button){
    ...
    try{
      ...
      if(errors.length){console.warn('V413 certificate merge skipped',errors);showToast(`สร้าง PDF แล้ว แต่แนบ Certificate ไม่สำเร็จ ${errors.length} ไฟล์`);}else showToast(`สร้าง PDF ชุดอบรมแล้ว • แนบ Certificate ${certificates.length} ไฟล์`);
    }catch(error){console.error('V413 annual training PDF',error);showToast(`สร้าง PDF ไม่สำเร็จ: ${error?.message||String(error)}`);}finally{frame?.remove();if(button){button.disabled=false;button.textContent=originalText||'Export PDF ชุดอบรมประจำปี';}}
  }
