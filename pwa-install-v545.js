/* CNMI Staff Planner PWA registration + install prompt — V545 */
(() => {
  'use strict';
  ...
  function ensureAuthButton() {
    ...
    if (!authCard || authCard.querySelector('[data-cnmi-pwa-auth-install="v545"]')) return;
    const wrapper = document.createElement('div');
    wrapper.dataset.cnmiPwaAuthInstall = 'v545';
  ...
  function ensureMenuButton() {
    ...
    if (!sidebarFoot || sidebarFoot.querySelector('[data-cnmi-pwa-menu-install="v545"]')) return;
    const button = createButton('pwa-install-btn--menu');
    button.dataset.cnmiPwaMenuInstall = 'v545';
  ...
  function createModal() {
    if (document.getElementById('pwaInstallModalV545')) return;
    const modal = document.createElement('div');
    modal.id = 'pwaInstallModalV545';
    modal.className = 'pwa-install-modal hidden';
    modal.setAttribute('role', 'dialog');
    modal.setAttribute('aria-modal', 'true');
    modal.setAttribute('aria-labelledby', 'pwaInstallTitleV545');
    modal.innerHTML = `
      <div class="pwa-install-dialog">
        <button type="button" class="pwa-install-close" aria-label="ปิด">×</button>
        <h3 id="pwaInstallTitleV545">ติดตั้ง CNMI Staff Planner</h3>
  ...
  function showModal(html) {
    createModal();
    const modal = document.getElementById('pwaInstallModalV545');
    if (!modal) return;
  ...
  async function handleInstallClick() {
    ...
    if (deferredInstallPrompt) {
      ...
      try {
        ...
        console.warn('[PWA V545] Install prompt failed:', error);
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      console.info('[PWA V545] Service Worker registered:', registration.scope);
    } catch (error) {
      console.warn('[PWA V545] Service Worker registration failed:', error);
    }
  }
