/* CNMI Staff Planner V518 — Activity recorder + participant UX + Dashboard newest-first
   Scope:
...
(function(){
  'use strict';
  if (window.__CNMI_V518_ACTIVITY_RECORDER_UI__) return;
  window.__CNMI_V518_ACTIVITY_RECORDER_UI__ = true;

  const VERSION = 'V518_ACTIVITY_RECORDER_PARTICIPANT_UI';
  ...
  const observer=new MutationObserver(()=>{
    ...
    document.querySelectorAll('.v518-note-wrap').forEach(wrap=>{
      ...
      if(!note||!btn) return;
      requestAnimationFrame(()=>{btn.hidden = note.scrollHeight <= note.clientHeight + 2 && !note.classList.contains('is-expanded');});
    });
  });
