...
(function(){
  'use strict';
  const VERSION='V503_RACE_DONOR_TIP_LINEBREAK';
  if(window.__CNMI_V503_RACE_DONOR_TIP_LINEBREAK__) return;
  window.__CNMI_V503_RACE_DONOR_TIP_LINEBREAK__=true;
  ...
  const phaseStore=window.__CNMI_V503_RACE_PHASES__||(window.__CNMI_V503_RACE_PHASES__={});
  ...
  function daytimePlan(date){
    ...
    return {id:'day',label:'กลางวัน',sub:'อิงตำแหน่งกลางวันจริง',leaderId,leaderNote,roles,roleNotes:{},roleTips:{R:'กลุ่มอพยพผู้บริจาค\nเหลือง — กำลังเจาะ / ยังต้องมีคนช่วยก่อนเคลื่อนย้าย\nเขียว — หลังถอดเข็มและเดินได้เอง'},warnings,teamCount:teamIds.length,note:'ลาครึ่งวัน: ใช้ผู้ปฏิบัติจาก Evacuate แทน R/A/C เดิมตลอดวัน เพื่อไม่ต้องเปลี่ยนบอร์ดกลางวัน; ผู้ลาครึ่งวันอยู่ Evacuate เฉพาะช่วงที่มาปฏิบัติงาน'};
  }
  ...
  function dutyPlan(date,kind){
    ...
    if(kind==='weekend-donor'){
      roleTips.R='กลุ่มอพยพผู้บริจาค\nเหลือง — กำลังเจาะ / ยังต้องมีคนช่วยก่อนเคลื่อนย้าย\nเขียว — หลังถอดเข็มและเดินได้เอง';
  ...
  if(typeof previous==='function'){
    const wrapped=function renderDashboardV503(){return decorateHtml(previous.apply(this,arguments));};
  ...
  window.cnmiV503Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate};
