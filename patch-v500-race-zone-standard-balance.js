 ...
 * - V500: one person can appear in only one R/A/C/E role per phase.
...
(function(){
  'use strict';
  const VERSION='V500_RACE_ZONE_STANDARD_BALANCE';
  if(window.__CNMI_V500_RACE_ZONE_STANDARD_BALANCE__) return;
  window.__CNMI_V500_RACE_ZONE_STANDARD_BALANCE__=true;
  ...
  const phaseStore=window.__CNMI_V500_RACE_PHASES__||(window.__CNMI_V500_RACE_PHASES__={});
  ...
  function daytimePlan(date){
    ...
    // C — Control: จำกัดกำลังหลัก 2 จุด (Blood Bank + Component Prep/Manual)
    let controlBlood=rowBy(['bbstockissue']);
    if(!controlBlood)controlBlood=rowBy(['bbreport','bbapprove']);
    take('C',controlBlood,controlBlood?`${controlBlood.label} · Blood Bank`:'' );
    let controlManual=rowBy(['bbmanual4','bbmanual3','bbmanual2','bbmanual1']);
    take('C',controlManual,controlManual?`${controlManual.label} · Component Prep`:'' );
    ...
    if(roles.C.length<2)warnings.push('Control มีผู้รับผิดชอบน้อยกว่า 2 จุด — ตรวจ Blood Bank และ Component Prep');
    ...
    return {id:'day',label:'กลางวัน',sub:'อิงตำแหน่งกลางวันจริง',leaderId,leaderNote,roles,roleNotes:{},warnings,teamCount:teamIds.length,note:'หัวหน้าทีมแยกจาก R/A/C/E · Alarm แยกด้านหน้า/ด้านหลัง · Control จำกัด 2 จุดหลัก · แพทย์ Consult ที่อยู่หน้างานช่วย Rescue · บุคลากรที่เหลือเป็น Evacuate'};
  }
  ...
  function dutyPlan(date,kind){
    ...
    const roles=emptyRoles(),roleNotes={},warnings=[];
    ...
    return {id:kind,label,sub,leaderId:leader.id,leaderNote:leader.note,roles,roleNotes,warnings,teamCount:included.length,note:[note,leaderRule].filter(Boolean).join(' · ')};
  }
  ...
  function roleHtml(role,people,note=''){
  ...
  function planHtml(plan,date){
    return `<div class="v497-plan" data-v497-plan="${esc(plan.id)}">
      ...
      <div class="v497-role-grid">${ROLE_ORDER.map(r=>roleHtml(r,plan.roles[r]||[],plan.roleNotes?.[r]||'')).join('')}</div>
  ...
  function copyPlan(date,id){
    ...
    const list=r=>(p.roles[r]||[]).map(x=>staffName(x.staff_id)).join(', ')||(p.roleNotes?.[r]||'-');
  ...
  window.cnmiV500Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate};
