...
(function(){
  'use strict';
  const VERSION = 'V348_OT_TRADE_RATE_TABS_POPUP';
  if (window.__CNMI_V348_OT_TRADE_RATE_TABS_POPUP__) return;
  window.__CNMI_V348_OT_TRADE_RATE_TABS_POPUP__ = true;
  ...
  function breakdown(row){
    ...
    if(!info)return base;
    return {...base,hrHours:info.claimHours,rateType:info.receiverType,tradeInfo:info,isTradeRate:true};
  }
  ...
  function detailRows(rows){
    ...
    const body=rows.map(row=>{
      const n=breakdown(row),info=n.tradeInfo;
      return `<tr><td>${esc(fmtDate(row.work_date))}</td><td>${esc(timeText(row))}</td><td><b>${esc(row.reason||'-')}</b>${row.note?`<br><span class="muted">${esc(row.note)}</span>`:''}${tradeExplain(info)}</td><td><b>${hours(n.actualHours)}</b></td><td><b>${hours(n.hrHours)}</b>${info?'<br><span class="badge purple">แปลงตามเรทที่ซื้อ</span>':''}</td><td>${claimStatus(row)}</td></tr>`;
    }).join('');
    const cards=rows.map(row=>{
      const n=breakdown(row),info=n.tradeInfo;
      return `<article class="v348-ot-card"><div class="v348-card-head"><b>${esc(fmtDate(row.work_date))}</b>${claimStatus(row)}</div><div><b>${esc(row.reason||'-')}</b><div class="muted">${esc(timeText(row))}${row.note?` • ${esc(row.note)}`:''}</div></div>${tradeExplain(info)}<div class="v348-hour-pair"><span>ชั่วโมงจริง <b>${hours(n.actualHours)}</b></span><span>ชั่วโมงเบิก HR <b>${hours(n.hrHours)}</b></span></div></article>`;
    }).join('');
  ...
  async function refreshDetail(root,sid,month){
    ...
    const loaded=await ensureTrades(month,sid);
    if(loaded?.loaded&&document.body.contains(root))updateDetailRoot(root,sid,month);
  }
  ...
  function queueFullRefresh(){
    ...
    setTimeout(async()=>{
      ...
      const result=await ensureTrades(month,currentSid());
      if(result?.loaded&&typeof renderPage==='function'){
  ...
  style.textContent=`
    ...
    @media(max-width:900px){
      .v348-desktop-detail{display:none}.v348-mobile-detail{display:grid;gap:10px}.v348-trade-box{min-width:0}.v348-trade-grid{grid-template-columns:1fr 1fr}.v348-ot-detail-modal .modal-card{width:min(96vw,100%)!important;padding:16px!important}.v347-claim-equation{grid-template-columns:1fr 1fr!important}.v347-claim-equation .money{grid-column:1/-1}
    }
    @media(max-width:560px){.v348-staff-tabs{display:grid;grid-template-columns:1fr}.v348-staff-tabs button{white-space:normal}.v348-trade-grid,.v348-hour-pair{grid-template-columns:1fr}.v347-claim-equation{grid-template-columns:1fr!important}.v347-claim-equation .money{grid-column:auto}.v348-trade-box{font-size:13px}}
  `;
  document.head.appendChild(style);

  window.cnmiV348={version:VERSION,breakdown,tradeForOtRow,tradeClaimInfo,ensureTrades,detailRows,arrangeStaff};
