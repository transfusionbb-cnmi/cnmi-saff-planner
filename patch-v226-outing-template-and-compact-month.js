/* =========================
   V226 Outing Template + Compact Month Position
   - Outing-date template can contain Outing / Blood Bank / Manual / Donor Room slots.
...
(function(){
  'use strict';
  const VERSION = 'V226_OUTING_TEMPLATE_COMPACT_MONTH';
  if (window.__CNMI_V226_OUTING_TEMPLATE_COMPACT_MONTH__) return;
  window.__CNMI_V226_OUTING_TEMPLATE_COMPACT_MONTH__ = true;
  ...
  function stateSlot(){ if (!state.slotTemplateV224) state.slotTemplateV224 = { kind:'day', setNo:10, configs:null, loaded:false, loading:false }; return state.slotTemplateV224; }
  ...
  function sanitizeRows(rows, kind){
    ...
    return (Array.isArray(rows) ? rows : []).map((r, i) => {
      ...
      if (!code) return null;
      const zone = ZONES.includes(String(r?.zone || '').trim()) ? String(r.zone).trim() : (isOutSet ? 'ออกหน่วย' : 'Blood Bank');
  ...
  function slotManagerHtml226(){
    ...
    const kindOptions = `<option value="day" ${kind==='day'?'selected':''}>วันทำงานปกติ 10-14 คน</option><option value="outing" ${kind==='outing'?'selected':''}>วันที่ออกหน่วย</option>`;
    ...
    return `<div class="position-template-page v224-position-template-page v226-position-template-page">
      <div class="card wide-card v224-slot-crud-card"><div class="section-title"><div><h3>ชุด Slot ตำแหน่งกลางวัน</h3><p class="hint">จัดการ Slot ผ่านหน้าเว็บได้เลย: เพิ่ม / แก้ไข / ลบ / เรียงลำดับ แล้วบันทึกเป็นต้นทางเดียวของตารางรายวันและรายเดือน</p></div><div class="actions"><button class="ghost-btn" type="button" data-v226-refresh-config>รีเฟรชจากฐานข้อมูล</button><button class="primary-btn" type="button" data-v226-save-all>บันทึกทั้งหมดเป็นฐานตำแหน่ง</button></div></div>
        ...
        ${kind==='outing'?'<div class="notice soft-notice compact"><b>วันที่ออกหน่วย:</b> สามารถเพิ่ม Slot โซน Blood Bank / Manual / Donor Room รวมในชุดนี้ได้เลย ระบบจะจัดคนที่ติ๊กเข้าร่วมกิจกรรมลง Slot ออกหน่วย และจัดคนที่ไม่ได้ออกหน่วยลง Slot ห้องตามที่เพิ่มไว้</div>':''}
  ...
  function openSlotModal226(idx=null, copy=false){
    ...
    showModal(`<div class="v226-slot-modal"><h2>${editing && !copy ? 'แก้ไข Slot' : 'เพิ่ม Slot'}</h2><p class="hint">${isOut?'วันที่ออกหน่วย: เลือกโซนได้ทั้ง ออกหน่วย / Blood Bank / Manual / Donor Room':`${selectedSet()} คน`}</p>
  ...
  function saveSlotModal226(form){
    ...
    const zone = String(fd.get('zone') || '').trim() || (selectedKind()==='outing' ? 'ออกหน่วย' : 'Blood Bank');
    const isOutingZone = zone === 'ออกหน่วย';
  ...
  function bucketForCount(n){ const x = Math.max(10, Math.min(14, Number(n) || 10)); return DAY_SETS.reduce((best, v) => Math.abs(v - x) < Math.abs(best - x) ? v : best, 10); }
  ...
  function zoneOf(pos){
    const code = baseCode(pos?.code || pos?.position_code || ''); let z = String(pos?.zone || '').trim();
    if (z === 'ออกหน่วย') return 'ออกหน่วย'; if (z === 'Manual' || /^BB-Manual/i.test(code)) return 'Manual'; if (z === 'Donor Room' || /^DR-/i.test(code)) return 'Donor Room'; if (z === 'Blood Bank' || /^BB-/i.test(code)) return 'Blood Bank'; return z || 'Blood Bank';
  }
  function isRealCode(code){ const c = baseCode(code); return !!c && c !== 'รอตรวจสอบ' && c !== '-'; }
  function loadWeight(pos){ try { return positionLoadWeight(baseCode(pos.code || pos.position_code)); } catch (_) { return zoneOf(pos)==='ออกหน่วย' ? 1.1 : zoneOf(pos)==='Manual' ? 1.15 : 1; } }
  ...
  function scorePositionForStaff(staff, pos, ctx){
    ...
    if (['Blood Bank','Manual','Donor Room'].includes(room) && !(cur.byRoom[room] || 0)) score -= 420;
  ...
  window.cnmiV226 = { loadConfigs226, currentConfigs226, buildWeeklyRotationPlan226, renderPositionManagement226, renderPositionsPage226, outingTemplateSlots };
