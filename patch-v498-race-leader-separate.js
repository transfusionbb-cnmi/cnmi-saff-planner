...
(function(){
  'use strict';
  const VERSION='V498_RACE_LEADER_SEPARATE';
  if(window.__CNMI_V498_RACE_LEADER_SEPARATE__) return;
  window.__CNMI_V498_RACE_LEADER_SEPARATE__=true;
  ...
  const phaseStore=window.__CNMI_V498_RACE_PHASES__||(window.__CNMI_V498_RACE_PHASES__={});
  ...
  function daytimePlan(date){
    ...
    // V498: หัวหน้าทีมฉุกเฉินเป็นบทบาทคุมภาพรวม แยกจากผู้ปฏิบัติ R/A/C/E
    removeLeaderFromRoles(roles,leaderId);
    ROLE_ORDER.forEach(r=>{if(!roles[r].length)warnings.push(`${ROLE_META[r].title} ยังไม่มีผู้รับผิดชอบจากตำแหน่งกลางวัน (หัวหน้าทีมไม่นับซ้ำใน RACE)`);});
    return {id:'day',label:'กลางวัน',sub:'อิงตำแหน่งกลางวันจริง',leaderId,leaderNote,roles,warnings,teamCount:uniquePeople(availableRows.map(x=>x.person)).length,note:'หัวหน้าทีมฉุกเฉินแยกจากผู้ปฏิบัติ R/A/C/E เพื่อคุมภาพรวม และทุกคนยังต้องรู้ลำดับ R-A-C-E ครบทั้ง 4 ขั้น'};
  }
  ...
  function dutyPlan(date,kind){
    ...
    // V498: คนที่เป็นหัวหน้าทีมฉุกเฉินไม่ถูกจัดซ้ำลง R/A/C/E
    ...
    const firstWorking=(codes=[])=>{
      for(const code of codes){const a=get(code)[0];if(a)return a;}
      return working[0]||null;
    };

    if(kind==='weekend-donor'){
      // ช3A ดูผู้เจาะเก็บ = Rescue, ช3B/ช9 อยู่ flow ผู้บริจาค = Evacuate
      // ชบดที่เหลือแบ่ง Alarm/Control โดยไม่นับหัวหน้าทีมซ้ำ
      const alarm=firstWorking(['ชบด2','ชบด3','ช9','ช9-เคิก','ช9-MT','ช3B']);
      if(alarm)addRole(roles,'A',dutyPerson(alarm,'Alarm ประจำกะ'));
      const control=firstWorking(['ชบด3','ชบด2','ช3B','ช9','ช9-เคิก','ช9-MT']);
      if(control)addRole(roles,'C',dutyPerson(control,'Control ประจำกะ'));
      addAssignments('R',get('ช3A'));
      addAssignments('E',[...get('ช3B'),...get('ช9'),...get('ช9-เคิก'),...get('ช9-MT')]);
      if(!roles.R.length){
        const fallback=firstWorking(['ช3B','ชบด3','ชบด2']);
        if(fallback)addRole(roles,'R',dutyPerson(fallback,'สำรอง Rescue'));
      }
      if(!roles.E.length){
        const fallback=firstWorking(['ช9-เคิก','ช9-MT','ช3B','ช3A','ชบด3','ชบด2']);
        if(fallback)addRole(roles,'E',dutyPerson(fallback,'สำรอง Evacuate'));
      }
    }else{
      // นอกเวลาปกติมีเฉพาะเจ้าหน้าที่คลังเลือด: หัวหน้าทีมคุมภาพรวม
      // ผู้ปฏิบัติที่เหลือสามารถรับมากกว่า 1 RACE role ได้เมื่อกำลังคนน้อย
      const alarm=firstWorking(['ชบด2','ชบด3','ช4','ช4A','ช4B']);
      if(alarm)addRole(roles,'A',dutyPerson(alarm,'Alarm ประจำกะ'));
      addAssignments('C',[...get('ชบด2'),...get('ช4'),...get('ช4A'),...get('ช4B')]);
      addAssignments('R',get('ชบด3'));
      const evacBase=working.filter(a=>['ชบด1','ชบด2','ชบด3'].includes(dutyCode(a)));
      addAssignments('E',evacBase.length?evacBase:working,'ร่วมอพยพหลังหน้าที่หลัก');
      if(kind==='evening')addAssignments('E',[...get('ช4'),...get('ช4A'),...get('ช4B')],'ร่วมอพยพหลังหน้าที่หลัก');
      if(!roles.C.length){const a=firstWorking(['ชบด2','ชบด3','ช4','ช4A','ช4B']);if(a)addRole(roles,'C',dutyPerson(a,'สำรอง Control'));}
      if(!roles.R.length){const a=firstWorking(['ชบด3','ชบด2','ช4','ช4A','ช4B']);if(a)addRole(roles,'R',dutyPerson(a,'สำรอง Rescue'));}
      if(!roles.E.length&&working[0])addRole(roles,'E',dutyPerson(working[0],'สำรอง Evacuate'));
    }
    removeLeaderFromRoles(roles,leader.id);
    ROLE_ORDER.forEach(r=>{if(!roles[r].length)warnings.push(`${ROLE_META[r].title} ยังไม่มีผู้รับผิดชอบในเวรนี้ (หัวหน้าทีมไม่นับซ้ำใน RACE)`);});
    ...
    const leaderRule='หัวหน้าทีมฉุกเฉินแยกจากผู้ปฏิบัติ R/A/C/E';
    return {id:kind,label,sub,leaderId:leader.id,leaderNote:leader.note,roles,warnings,teamCount:included.length,note:[note,leaderRule].filter(Boolean).join(' · ')};
  }
  ...
  if(typeof previous==='function'){
    const wrapped=function renderDashboardV498(){return decorateHtml(previous.apply(this,arguments));};
  ...
  function showRaceInfo(){
    const body=`<div class="v497-info"><p class="hint">ใช้คำและหลักตามบอร์ด Code 01 ของโรงพยาบาล</p>
      ...
      <p class="v497-info-note">หัวหน้าทีมฉุกเฉินคุมภาพรวมและไม่ต้องมี R/A/C/E ซ้ำ ส่วนผู้ปฏิบัติทุกคนต้องเข้าใจ R-A-C-E ครบทั้ง 4 ขั้น</p></div>`;
  ...
  window.cnmiV498Race={version:VERSION,buildPlans,daytimePlan,dutyPlan,positionRole,selectedDate};
