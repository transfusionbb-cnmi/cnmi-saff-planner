...
async function init() {
  // V545: wait for one-time cleanup/reload if an older PWA worker controls the page.
  if (window.CNMI_V545_PWA_RECOVERY) {
    try {
      const recovery = await window.CNMI_V545_PWA_RECOVERY;
      if (recovery?.reloading) return;
    } catch (error) {
      console.warn('[V545] PWA recovery check failed:', error);
    }
  }
  ...
  // V545: Supabase is the only external dependency required for login.
  // Use cached bundle -> jsDelivr -> unpkg fallback so mobile startup never
  // waits forever on one CDN. Optional export libraries load later.
  if (!window.supabase?.createClient && window.CNMI_DEPENDENCY_READY?.supabase) {
    const loginSubmitV545 = $('loginForm')?.querySelector('button[type="submit"],input[type="submit"]');
    if (loginSubmitV545) loginSubmitV545.disabled = true;
    try {
      setBusy(true, 'กำลังเชื่อมต่อระบบ');
      await window.CNMI_DEPENDENCY_READY.supabase;
      if (loginSubmitV545) loginSubmitV545.disabled = false;
    } catch (err) {
      console.error('[V545] Supabase dependency failed to load', err);
      const warning = $('setupWarning');
      if (warning) {
        warning.classList.remove('hidden');
        warning.innerHTML = '<b>เชื่อมต่อระบบไม่สำเร็จ</b><br>ระบบ Login โหลดไม่ครบจากเครือข่าย กรุณากด “ลองใหม่”<div style="margin-top:10px"><button type="button" class="primary-btn" onclick="location.reload()">ลองใหม่</button></div>';
      }
      setBusy(false);
      return;
    } finally {
...
/* =========================
   V228 Weekly leave-aware position plan
...
(function(){
  'use strict';
  const VERSION_V228 = 'V228_WEEKLY_LEAVE_AWARE_POSITION_PLAN';
  if (window.__CNMI_V228_WEEKLY_LEAVE_AWARE_POSITION_PLAN__) return;
  window.__CNMI_V228_WEEKLY_LEAVE_AWARE_POSITION_PLAN__ = true;
