...
function renderDutyTradePanel(assignments) {
  const monthRows = state.tradeRequests.filter(r => {
    // The month filter is the duty month, never the month when the trade was created.
    // Resolve against the complete loaded roster because a completed trade can alter
    // the current-month assignment list while the request still points to that duty.
    const from = state.rosterAssignments.find(x => x.id === r.from_assignment_id);
    const to = state.rosterAssignments.find(x => x.id === r.to_assignment_id);
    return [from, to].some(x => String(x?.duty_date || '').slice(0, 7) === state.monthKey);
  });
  const staffFilter = state.tradeFilterStaff || '';
