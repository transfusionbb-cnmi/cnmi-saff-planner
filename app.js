...
const NAV_ITEMS = [
  ...
  { id: 'myTraining', icon: '📚', title: 'รายการอบรมของฉัน', subtitle: 'บันทึกผลการอบรมและ Export FM-CNHR-002', group: 'staff' },
  ...
  { id: 'trainingAdmin', icon: '🧾', title: 'ดูอบรมของบุคลากร', subtitle: 'ตรวจสอบ กรอง และ Export ประวัติการอบรม', group: 'admin' },
...
let state = {
  ...
  positionEligibility: [],
  positionDayStatus: [],
  trainingFilterStaff: '', trainingFilterFrom: '', trainingFilterTo: '', trainingFilterYear: '', trainingFilterStatus: '',
...
async function loadAllData() {
  ...
  const [staff, leaves, activities, rosterMonths, rosterAssignments, positions, attendance, otRequests, tradeRequests, hrChecks, auditLogs, holidays, incharges, positionEligibility, positionDayStatus, trainingRecords] = await Promise.all([
    ...
    sb.from('training_records').select('*').order('training_date', { ascending: false })
  ]);
  const packs = { staff, leaves, activities, rosterMonths, rosterAssignments, positions, attendance, otRequests, tradeRequests, hrChecks, auditLogs, holidays, incharges, positionEligibility, positionDayStatus, trainingRecords };
  ...
  state.trainingRecords = trainingRecords.data || [];
...
function renderPage() {
  ...
  const pages = {
    ...
    myTraining: renderMyTrainingPage,
    trainingAdmin: renderTrainingAdminPage,
    hr: renderHrPage,
...
function collectCalendarEvents() {
  ...
  state.leaves.filter(x => x.status !== 'cancelled').forEach(l => {
    ...
    days.forEach(date => {
      ...
      const periodSuffix = l.leave_period && l.leave_period !== 'เต็มวัน' ? ` (${l.leave_period})` : '';
      events.push({ date, type: lt, title: `${l.type}: ${staffNick(l.staff_id)}${periodSuffix}${reasonSuffix}${hrChecked ? ' ✓ ตรวจ HR แล้ว' : ''}`, raw: l, hrChecked, reason });
    });
  });
  ...
  return `<div class="grid grid-2">
    <div class="card">
      ...
      <div class="profile-info-grid">
        ...
        <div><span class="muted">วันเริ่มงาน</span><b>${p.start_work_date ? formatThaiDate(p.start_work_date) : 'ยังไม่ระบุ'}</b></div>
    ...
    <div class="grid grid-2">
      <div class="card">
        ...
        <form id="activityForm" class="form-grid">
          ...
          <label class="wide training-import-option"><span><input type="checkbox" name="import_fm_cnhr_002" value="true" ${(editing?.import_fm_cnhr_002 || editing?.training_form_type === 'FM-CNHR-002') ? 'checked' : ''}> นำเข้าแบบฟอร์ม FM-CNHR-002</span><small class="hint">ไม่บังคับติ๊ก ติ๊กเมื่อกิจกรรมนี้ต้องบันทึกเป็นประวัติการอบรมของผู้เข้าร่วม</small></label>
...
function renderUsersPage() {
  ...
  const userForm = (s) => `<div class="admin-user-card admin-user-single-card" data-staff-row="${s.id}">
    ...
    <div class="admin-user-form">
      ...
      <label>วันเริ่มงาน <input data-field="start_work_date" type="date" value="${escapeHtml(s.start_work_date || '')}"></label>
    ...
    <div class="card eligibility-staff-panel">
      ...
      <details class="add-user-details">
        ...
        <form id="newStaffForm" class="form-grid compact-form">
          ...
          <label>วันเริ่มงาน <input name="start_work_date" type="date"></label>
...
async function handleSubmit(e) {
  ...
  if (e.target.id === 'trainingRecordForm') { e.preventDefault(); await saveTrainingRecord(e.target); }
...
async function handleClick(e) {
  ...
  if (t.dataset.trainingEdit) { const r = (state.trainingRecords || []).find(x => String(x.id) === String(t.dataset.trainingEdit)); if (r) showModal(`<h2>บันทึกรายการอบรม</h2>${trainingForm(r)}`); return; }
  if (t.dataset.trainingExport) { const rows = t.dataset.trainingExport === 'mine' ? trainingRowsForViewer(false) : trainingRowsForViewer(true); exportTrainingPdf(rows, t.dataset.trainingExport === 'mine' ? 'รายการอบรมของฉัน FM-CNHR-002' : 'ประวัติการอบรมบุคลากร FM-CNHR-002'); return; }
...
function handleChange(e) {
  ...
  if (e.target.id === 'trainingFilterStaff') { state.trainingFilterStaff = e.target.value; renderPage(); }
  if (e.target.id === 'trainingFilterFrom') { state.trainingFilterFrom = e.target.value; renderPage(); }
  if (e.target.id === 'trainingFilterTo') { state.trainingFilterTo = e.target.value; renderPage(); }
  if (e.target.id === 'trainingFilterYear') { state.trainingFilterYear = e.target.value; renderPage(); }
  if (e.target.id === 'trainingFilterStatus') { state.trainingFilterStatus = e.target.value; renderPage(); }
...
async function saveActivity(form) {
  ...
  const row = {
    ...
    import_fm_cnhr_002: fd.get('import_fm_cnhr_002') === 'true',
    updated_by: currentStaffId()
  };
  ...
  const res = id ? await sb.from('activity_events').update(row).eq('id', id) : await sb.from('activity_events').insert({ ...row, created_by: currentStaffId() }).select('id').single();
  if (res.error) return showToast(friendlyDbError(res.error));
  const activityId = id || res.data?.id;
  if (activityId) {
    await sb.from('training_records').delete().eq('activity_id', activityId);
    if (row.import_fm_cnhr_002 && participants.length) {
      const trainingRows = participants.map(staff_id => ({
        activity_id: activityId, staff_id, training_date: row.start_date, end_date: row.end_date,
        title: row.title, organizer: row.owner_id, location: row.location, duration: row.start_date === row.end_date ? `${row.start_time || ''}-${row.end_time || ''}` : null,
        form_type: 'FM-CNHR-002', status: 'รอกรอกข้อมูล', created_by: currentStaffId(), updated_by: currentStaffId()
      }));
      const tr = await sb.from('training_records').insert(trainingRows);
      if (tr.error) return showToast('บันทึกกิจกรรมแล้ว แต่สร้างรายการอบรมไม่สำเร็จ: ' + friendlyDbError(tr.error));
    }
  }
...
async function saveStaffUsers() {
  ...
  Array.from(document.querySelectorAll('[data-staff-row]')).forEach(tr => {
    ...
    rowsById.set(id, {
      ...
      start_work_date: valueOr('start_work_date', original.start_work_date || '') || null,
...
async function saveNewStaff(form) {
  ...
  const row = {
    ...
    start_work_date: fd.get('start_work_date') || null,
...
(function () {
  ...
  window.renderPage = function renderPageV57() {
    ...
    const pages = {
      ...
      activities: renderActivitiesPage, myTraining: renderMyTrainingPage, trainingAdmin: renderTrainingAdminPage, hr: renderHrPage, hrSummary: renderHrSummaryPage, scheduler: renderSchedulerPage,
...
/* V395: FM-CNHR-002 is a record attached to an existing activity. */
function trainingRowsForViewer(admin) {
  const me = currentStaffId();
  return (state.trainingRecords || []).filter(r => admin || String(r.staff_id) === String(me));
}
function trainingStatus(r) {
  if (r.status) return r.status;
  if (r.result || r.application_use || r.certificate_path) return 'กรอกข้อมูลแล้ว';
  return 'รอกรอกข้อมูล';
}
function trainingRecordCard(r, allowEdit=true) {
  const st = state.staff.find(x => String(x.id) === String(r.staff_id)) || {};
  return `<div class="mobile-card training-record-card">
    <div class="section-title"><div><h3>${escapeHtml(r.title || '-')}</h3><span class="muted">${escapeHtml(st.nickname || st.full_name || '-')}</span></div>${badge(trainingStatus(r), trainingStatus(r)==='กรอกข้อมูลแล้ว'?'green':'yellow')}</div>
    <div><b>วันที่อบรม:</b> ${formatThaiDate(r.training_date)}${r.end_date && r.end_date !== r.training_date ? ` - ${formatThaiDate(r.end_date)}` : ''}</div>
    <div><b>หน่วยงาน/ผู้รับผิดชอบ:</b> ${escapeHtml(staffNick(r.organizer) || '-')}</div>
    <div><b>ผล/สิ่งที่ได้รับ:</b> ${escapeHtml(r.result || '-')}</div>
    <div><b>การนำความรู้ไปใช้:</b> ${escapeHtml(r.application_use || '-')}</div>
    <div>${r.certificate_path ? badge('แนบ Certificate แล้ว','green') : badge('ยังไม่แนบ Certificate','blue')}</div>
    ${allowEdit ? `<button class="primary-btn" data-training-edit="${r.id}">กรอก/แก้ไขข้อมูล</button>` : ''}
  </div>`;
}
function trainingForm(r) {
  return `<form id="trainingRecordForm" class="form-grid" data-training-id="${r.id}">
    <label class="wide">หัวข้ออบรม <input value="${escapeHtml(r.title || '')}" disabled></label>
    <label>วันที่อบรม <input value="${escapeHtml(r.training_date || '')}" disabled></label>
    <label>แบบฟอร์ม <input value="FM-CNHR-002" disabled></label>
    <label class="wide">ผล/สิ่งที่ได้รับ <textarea name="result" placeholder="กรอกผลหรือสิ่งที่ได้รับ">${escapeHtml(r.result || '')}</textarea></label>
    <label class="wide">การนำความรู้ไปใช้ <textarea name="application_use" placeholder="กรอกการนำความรู้ไปใช้ในการปฏิบัติงาน">${escapeHtml(r.application_use || '')}</textarea></label>
    <label class="wide">Certificate <input name="certificate" type="file" accept="application/pdf,image/*"><span class="hint">ไม่บังคับแนบไฟล์ และไม่มีไฟล์ก็ Export PDF ได้</span></label>
    <button class="primary-btn wide" type="submit">บันทึกข้อมูลอบรม</button>
  </form>`;
}
function renderMyTrainingPage() {
  const rows = trainingRowsForViewer(false);
  return `<div class="card"><div class="section-title"><div><h3>รายการอบรมของฉัน</h3><p class="hint">แสดงเฉพาะรายการที่ Admin ติ๊ก “นำเข้าแบบฟอร์ม FM-CNHR-002” และเลือกชื่อคุณเป็นผู้เข้าร่วม</p></div><button class="ghost-btn" data-training-export="mine">Export PDF</button></div>
    ${rows.length ? `<div class="mobile-cards always-cards">${rows.map(r => trainingRecordCard(r)).join('')}</div>` : empty('ยังไม่มีรายการอบรมของคุณ')}</div>`;
}
function renderTrainingAdminPage() {
  if (!isAdmin()) return noPermission();
  const f = state;
  let rows = trainingRowsForViewer(true).filter(r => !f.trainingFilterStaff || String(r.staff_id) === String(f.trainingFilterStaff))
    .filter(r => !f.trainingFilterFrom || String(r.training_date) >= f.trainingFilterFrom)
    .filter(r => !f.trainingFilterTo || String(r.training_date) <= f.trainingFilterTo)
    .filter(r => !f.trainingFilterYear || String(r.training_date).slice(0,4) === f.trainingFilterYear)
    .filter(r => !f.trainingFilterStatus || trainingStatus(r) === f.trainingFilterStatus);
  const years = [...new Set((state.trainingRecords || []).map(r => String(r.training_date || '').slice(0,4)).filter(Boolean))].sort().reverse();
  return `<div class="card"><div class="section-title"><div><h3>ดูอบรมของบุคลากร</h3><p class="hint">ตรวจสอบและ Export รายการ FM-CNHR-002 ตามสิทธิ์ Admin/แพทย์</p></div><button class="ghost-btn" data-training-export="admin">Export PDF</button></div>
    <div class="toolbar compact-filter"><label>บุคลากร <select id="trainingFilterStaff"><option value="">ทุกคน</option>${orderedStaff(state.staff).map(s => `<option value="${s.id}" ${f.trainingFilterStaff===s.id?'selected':''}>${escapeHtml(s.nickname || s.full_name)}</option>`).join('')}</select></label>
    <label>จากวันที่ <input id="trainingFilterFrom" type="date" value="${f.trainingFilterFrom}"></label><label>ถึงวันที่ <input id="trainingFilterTo" type="date" value="${f.trainingFilterTo}"></label>
    <label>ปี <select id="trainingFilterYear"><option value="">ทุกปี</option>${years.map(y => `<option ${f.trainingFilterYear===y?'selected':''}>${y}</option>`).join('')}</select></label>
    <label>สถานะ <select id="trainingFilterStatus"><option value="">ทุกสถานะ</option><option ${f.trainingFilterStatus==='รอกรอกข้อมูล'?'selected':''}>รอกรอกข้อมูล</option><option ${f.trainingFilterStatus==='กรอกข้อมูลแล้ว'?'selected':''}>กรอกข้อมูลแล้ว</option></select></label></div>
    ${rows.length ? `<div class="mobile-cards always-cards">${rows.map(r => trainingRecordCard(r, false)).join('')}</div>` : empty('ไม่พบข้อมูลตามตัวกรอง')}</div>`;
}
async function saveTrainingRecord(form) {
  const id = form.dataset.trainingId;
  const original = (state.trainingRecords || []).find(r => String(r.id) === String(id));
  if (!original || (String(original.staff_id) !== String(currentStaffId()) && !isAdmin())) return showToast('ไม่มีสิทธิ์แก้ไขรายการนี้');
  const fd = new FormData(form);
  const patch = { result: String(fd.get('result') || '').trim(), application_use: String(fd.get('application_use') || '').trim(), status: (fd.get('result') || fd.get('application_use')) ? 'กรอกข้อมูลแล้ว' : 'รอกรอกข้อมูล', updated_by: currentStaffId(), updated_at: new Date().toISOString() };
  const file = fd.get('certificate');
  try { if (file && file.size) patch.certificate_path = await uploadFile(file, 'training-certificates'); } catch (err) { return showToast('อัปโหลด Certificate ไม่สำเร็จ: ' + err.message); }
  const { error } = await sb.from('training_records').update(patch).eq('id', id);
  if (error) return showToast(friendlyDbError(error));
  closeModal(); await loadAllData(); renderPage(); showToast('บันทึกข้อมูลอบรมแล้ว');
}
function exportTrainingPdf(rows, title) {
  const html = `<html><head><meta charset="utf-8"><title>${title}</title><style>body{font-family:Arial,sans-serif;padding:24px}h1{font-size:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #999;padding:7px;text-align:left;vertical-align:top}th{background:#eee}</style></head><body><h1>${title}</h1><table><thead><tr><th>บุคลากร</th><th>หลักสูตร/เรื่อง</th><th>วันที่</th><th>ผล/สิ่งที่ได้รับ</th><th>การนำไปใช้</th><th>สถานะ</th></tr></thead><tbody>${rows.map(r => `<tr><td>${escapeHtml(staffNick(r.staff_id))}</td><td>${escapeHtml(r.title || '')}</td><td>${escapeHtml(r.training_date || '')}</td><td>${escapeHtml(r.result || '')}</td><td>${escapeHtml(r.application_use || '')}</td><td>${escapeHtml(trainingStatus(r))}</td></tr>`).join('')}</tbody></table><script>window.onload=()=>window.print()</script></body></html>`;
  const w = window.open('', '_blank'); if (!w) return showToast('กรุณาอนุญาตการเปิดหน้าต่างใหม่เพื่อ Export PDF'); w.document.write(html); w.document.close();
}
